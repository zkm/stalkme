...
(function ($) {
      ...
      var outputElement = $(this),
        ...
        finished = function (inputdata) {
          ...
          var items = data.items,
            ...
            // for loop
            length =
              items.length < settings.limit ? items.length : settings.limit,
            i = 0,
            item,
        ...
        load = function () {
          ...
          for (; i < j; i++) {
            ...
            if (
              $.fn.lifestream.feeds[config.service] &&
  ...
  $.fn.lifestream.createYqlUrl = function (query) {
    return (
      "http://query.yahooapis.com/v1/public/yql?q=__QUERY__&env=" +
    ...
    var template = $.extend(
        {},
        {
          ...
          pullrequest_fulfilled:
            'fulfilled a pull request at <a href="http://bitbucket.org/${owner}/${name}/">${owner}/${name}</a>',
          pullrequest_rejected:
            'rejected a pull request at <a href="http://bitbucket.org/${owner}/${name}/">${owner}/${name}</a>',
          pullrequest_created:
            'created a pull request at <a href="http://bitbucket.org/${owner}/${name}/">${owner}/${name}</a>',
          create:
            'created a new project at <a href="http://bitbucket.org/${owner}/${name}/">${owner}/${name}</a>',
...
(function ($) {
  $.fn.lifestream.feeds.bitly = function (config, callback) {
    var template = $.extend(
      ...
      {
        created:
          'created URL <a href="${short_url}" title="${title}">' +
          "${short_url}</a>",
      },
      config.template
    );
    ...
    $.ajax({
      url: $.fn.lifestream.createYqlUrl(
        "select data.short_url, data.created, " +
...
(function ($) {
  $.fn.lifestream.feeds.blogger = function (config, callback) {
    ...
    $.ajax({
      url: $.fn.lifestream.createYqlUrl(
        'select * from xml where url="http://' +
          config.user +
...
(function ($) {
  $.fn.lifestream.feeds.digg = function (config, callback) {
    ...
    $.ajax({
      url:
        "http://services.digg.com/2.0/user.getActivity?username=" +
        config.user +
        "&type=javascript",
      dataType: "jsonp",
      success: function (data) {
...
(function ($) {
  $.fn.lifestream.feeds.gimmebar = function (config, callback) {
    ...
    $.ajax({
      url:
        "https://gimmebar.com/api/v0/public/assets/" +
        config.user +
        ".js?jsonp_callback=?",
      dataType: "json",
      success: function (data) {
...
(function ($) {
  $.fn.lifestream.feeds.googleplus = function (config, callback) {
    var template = $.extend(
        ...
        {
          posted:
            '<a href="${actor.url}">${actor.displayName}</a> has posted a new entry <a href="${url}" ' +
...
(function ($) {
  $.fn.lifestream.feeds.googlereader = function (config, callback) {
    var template = $.extend(
        ...
        {
          starred:
            'shared <a href="{{if link.href}}${link.href}' +
...
(function ($) {
  $.fn.lifestream.feeds.hypem = function (config, callback) {
    ...
    var template = $.extend(
      ...
      {
        loved:
          'loved <a href="http://hypem.com/item/${mediaid}">${title}</a> by <a href="http://hypem.com/artist/${artist}">${artist}</a>',
        history:
          'listened to <a href="http://hypem.com/item/${mediaid}">${title}</a> by <a href="http://hypem.com/artist/${artist}">${artist}</a>',
      },
      config.template
    );
      ...
      success: function (data) {
        ...
        if (data && j > 0) {
          for (; i < j; i++) {
            ...
            output.push({
              date: new Date(
                (config.type === "history" ? item.dateplayed : item.dateloved) *
                  1000
              ),
              config: config,
              html: $.tmpl(
                config.type === "history" ? template.history : template.loved,
                item
              ),
            });
          }
...
(function ($) {
  $.fn.lifestream.feeds.iusethis = function (config, callback) {
    ...
    $.ajax({
      url: $.fn.lifestream.createYqlUrl(
        "select * from xml where " +
          'url="http://iphone.iusethis.com/user/feed.rss/' +
          config.user +
          '" or ' +
...
(function ($) {
  $.fn.lifestream.feeds.librarything = function (config, callback) {
    var template = $.extend(
        ...
        {
          book:
            'added <a href="http://www.librarything.com/work/book/${book.book_id}"' +
    ...
    $.ajax({
      url:
        "http://www.librarything.com/api_getdata.php?booksort=entry_REV&userid=" +
        config.user,
      dataType: "jsonp",
      success: function (data) {
...
(function ($) {
  $.fn.lifestream.feeds.picplz = function (config, callback) {
    ...
    $.ajax({
      url:
        "http://picplz.com/api/v2/user.json?username=" +
        config.user +
        "&include_pics=1",
      dataType: "jsonp",
      success: function (data) {
...
(function ($) {
  $.fn.lifestream.feeds.pinboard = function (config, callback) {
    ...
    $.ajax({
      url: $.fn.lifestream.createYqlUrl(
        "select * from xml where " +
          'url="http://feeds.pinboard.in/rss/u:' +
          config.user +
          '"'
      ),
      dataType: "jsonp",
...
(function ($) {
  $.fn.lifestream.feeds.rss = function (config, callback) {
    ...
    $.ajax({
      url: $.fn.lifestream.createYqlUrl(
        'select * from xml where url="' + config.user + '"'
      ),
      dataType: "jsonp",
      success: function (data) {
        callback(parseReader(data));
...
(function ($) {
  $.fn.lifestream.feeds.snipplr = function (config, callback) {
    ...
    $.ajax({
      url: $.fn.lifestream.createYqlUrl(
        "select * from xml where " +
          'url="http://snipplr.com/rss/users/' +
          config.user +
          '"'
      ),
      dataType: "jsonp",
...
(function ($) {
  $.fn.lifestream.feeds.tumblr = function (config, callback) {
    ...
    $.ajax({
      url: $.fn.lifestream.createYqlUrl(
        "select *" + ' from tumblr.posts where username="' + config.user + '"'
      ),
      dataType: "jsonp",
      success: function (data) {
  ...
  $.fn.lifestream.feeds.wikipedia = function (config, callback) {
    ...
    $.ajax({
      url:
        ...
        config.user +
        "&list=usercontribs&ucdir=older&format=json",
      dataType: "jsonp",
      success: function (data) {
        var output = [],
          i = 0,
...
(function ($) {
  $.fn.lifestream.feeds.youtube = function (config, callback) {
    var template = $.extend(
        ...
        {
          favorited:
            'favorited <a href="${video.player.default}" ' +
